...
@Config
@TeleOp(name = "LateralVelocityTuning", group = "Tuning")
public class LateralVelocityTuning extends LinearOpMode implements Tuning {
    ...
    public void runOpMode() {
        ...
        try {
            ...
            mLogger = new LogManager(null,FtcDashboard.getInstance(),"lateral-velocity-tuning");
            ...
            FtcDashboard.getInstance().removeConfigVariable(LateralVelocityTuning.class.getSimpleName(),"SAVE");

            String description = "<p style=\"font-weight: bold; font-size: 14px\"> ------------------------- </p>" +
                    "<p style=\"font-weight: bold; font-size: 14px\">The robot will run to the right at 1 power until it reaches " + DISTANCE + " inches forward</p>" +
            ...
            while(opModeIsActive()) {
                ...
                if(!mShallEnd) {
                    if (Math.abs(mUpdater.getPose().getY()) > DISTANCE) {
                        mShallEnd = true;
                    } else {
                        double currentVelocity = Math.abs(MathFunctions.dotProduct(mUpdater.getVelocity(), new Vector(1, Math.PI/2)));
                ...
                if(mShallEnd) {
                    ...
                    stopMotors();
                    FtcDashboard.getInstance().addConfigVariable(LateralVelocityTuning.class.getSimpleName(),"SAVE",mShallSave);

                    if(mShallSave.get()) {
                        FollowerConstants.yMovement = average;
                    }
                }
            ...
            mConfiguration.write(Environment.getExternalStorageDirectory().getPath()
                    + "/FIRST/lateral-velocity-tuning.json");
            mLogger.info("Updated configuration saved. You may retrieve it using <b>adb pull /sdcard/FIRST/lateral-velocity-tuning.json</b>");
            mLogger.update();
            mLogger.stop();
