 ...
 */
@Config
@Autonomous (name = "Curved Back And Forth Tuning", group = "PIDF Testing")
public class CurvedBackAndForthTuning extends LinearOpMode implements Tuning {
    ...
    public void runOpMode() {
        ...
        try {
            ...
            mLogger = new LogManager(null,FtcDashboard.getInstance(),"mecanum-drive-curve-back-and-forth-tuning");
            ...
            mBackwards = new Path(new BezierCurve(new Point(Math.abs(DISTANCE),DISTANCE, Point.CARTESIAN), new Point(Math.abs(DISTANCE),0, Point.CARTESIAN), new Point(0,0, Point.CARTESIAN)));
            mBackwards.setReversed(true);

            mDrive.followPath(mForwards);

            String description = "<p style=\"font-weight: bold; font-size: 14px\"> ------------------------- </p>" +
                    "<p style=\"font-weight: bold; font-size: 14px\"> This will run the robot in a curve going " + DISTANCE + " inches" +
            ...
            mConfiguration.write(Environment.getExternalStorageDirectory().getPath()
                    + "/FIRST/mecanum-drive-curve-back-and-forth-tuning.json");
            mLogger.info("Updated configuration saved. You may retrieve it using <b>adb pull /sdcard/FIRST/mecanum-drive-curve-back-and-forth-tuning.json</b>");
            mLogger.update();
            mLogger.stop();
        }
        catch(Exception e) {
