...
@Config
@TeleOp(name = "ForwardTuning", group = "Tuning")
public class ForwardTuning extends LinearOpMode implements Tuning {
    ...
    public void runOpMode() {
        ...
        try {
            ...
            mLogger = new LogManager(null,FtcDashboard.getInstance(),"localizer-forward-tuning");
            ...
            for (Map.Entry<String, Boolean> selected : mLocalizerSelection.entrySet()) {
                ...
                FtcDashboard.getInstance().addConfigVariable(ForwardTuning.class.getSimpleName(),selected.getKey(),provider);
            }
            mShallSave = new SaveProvider();
            FtcDashboard.getInstance().addConfigVariable(ForwardTuning.class.getSimpleName(),"SAVE",mShallSave);

            String description = "<p style=\"font-weight: bold; font-size: 14px\"> ------------------------- </p>" +
                    "<p style=\"font-weight: bold; font-size: 14px\"> Pull your robot forward " + DISTANCE + " inches. Your forward ticks to inches will be shown on the telemetry. </p>";
            ...
            while(opModeIsActive()) {
                ...
                description = "<p style=\"font-weight: bold; font-size: 14px\"> ------------------------- </p>" +
                        "<p style=\"font-weight: bold; font-size: 14px\"> FORWARD TUNING </p>" +
                        "<p style=\"font-size: 12px\"> The multiplier will display what your forward multiplies should be to scale your current distance to " + DISTANCE + " inches. </p>" ;
                mLogger.info(LogManager.Target.DASHBOARD,description);
                ...
                if(!Objects.equals(currentLocalizer, mCurrentLocalizer))  {
                    ...
                    if(mShallSave.get()) { mSelectedLocalizer.setForwardMultiplier(mMultiplier); }
                ...
                if(mSelectedUpdater != null) { mSelectedUpdater.update(); }
                if(mDashboardPoseTracker != null) { mDashboardPoseTracker.update(); }
                mMultiplier = DISTANCE / (mSelectedUpdater.getPose().getX() / mSelectedUpdater.getLocalizer().getForwardMultiplier());
            ...
            mConfiguration.write(Environment.getExternalStorageDirectory().getPath()
                    + "/FIRST/localization-forward-tuning.json");
            mLogger.info("Updated configuration saved. You may retrieve it using <b>adb pull /sdcard/FIRST/localization-forward-tuning.json</b>");
            mLogger.update();
        }
        catch(Exception e) {
