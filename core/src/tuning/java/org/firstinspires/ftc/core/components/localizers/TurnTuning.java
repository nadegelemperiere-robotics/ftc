...
public class TurnTuning extends LinearOpMode implements Tuning {
    ...
    public void runOpMode() {
        ...
        try {
            ...
            mLogger = new LogManager(null,FtcDashboard.getInstance(),"localizer-turn-tuning");
            ...
            for (Map.Entry<String, Boolean> selected : mLocalizerSelection.entrySet()) {
                ...
                FtcDashboard.getInstance().addConfigVariable(TurnTuning.class.getSimpleName(),selected.getKey(),provider);
            }
            mShallSave = new SaveProvider();
            FtcDashboard.getInstance().addConfigVariable(TurnTuning.class.getSimpleName(),"SAVE",mShallSave);

            String description = "<p style=\"font-weight: bold; font-size: 14px\"> ------------------------- </p>" +
                    "<p style=\"font-weight: bold; font-size: 14px\"> Turn your robot " + ANGLE + " radians. Your turn ticks to inches will be shown on the telemetry. </p>";
            ...
            while(opModeIsActive()) {
                ...
                description = "<p style=\"font-weight: bold; font-size: 14px\"> ------------------------- </p>" +
                        "<p style=\"font-weight: bold; font-size: 14px\"> TURN TUNING </p>" +
                        "<p style=\"font-size: 12px\"> The multiplier will display what your turn ticks to inches should be to scale your current angle to " + ANGLE + " radians. </p>" ;
                mLogger.info(LogManager.Target.DASHBOARD,description);
                ...
                if(!Objects.equals(currentLocalizer, mCurrentLocalizer))  {
                    ...
                    if(mShallSave.get()) { mSelectedLocalizer.setTurningMultiplier(mMultiplier); }
                ...
                mMultiplier = ANGLE / (mSelectedUpdater.getTotalHeading() / mSelectedUpdater.getLocalizer().getTurningMultiplier());
            ...
            mConfiguration.write(Environment.getExternalStorageDirectory().getPath()
                    + "/FIRST/localization-turn-tuning.json");
            mLogger.info("Updated configuration saved. You may retrieve it using <b>adb pull /sdcard/FIRST/localization-turn-tuning.json</b>");
            mLogger.update();
        }
        catch(Exception e) {
    ...
    private void logLocalizerState(LogManager logger) {
        ...
        if (mSelectedLocalizer != null) {
            ...
            logger.info("-----> Multiplier : " + mSelectedLocalizer.getTurningMultiplier());
