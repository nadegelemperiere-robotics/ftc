...
@Config
@TeleOp(name = "LateralZeroPowerAccelerationTuning", group = "Tuning")
public class LateralZeroPowerAccelerationTuning extends LinearOpMode implements Tuning {
    ...
    public void runOpMode() {
        ...
        try {
            ...
            mLogger = new LogManager(null,FtcDashboard.getInstance(),"lateral-zero-acceleration-tuning");
            ...
            FtcDashboard.getInstance().removeConfigVariable(LateralZeroPowerAccelerationTuning.class.getSimpleName(),"SAVE");

            String description = "<p style=\"font-weight: bold; font-size: 14px\"> ------------------------- </p>" +
                    "<p style=\"font-weight: bold; font-size: 14px\">The robot will run to the right until it reaches " + VELOCITY + " inches per second</p>" +
            ...
            while(opModeIsActive()) {
                ...
                Vector heading = new Vector(1.0, mUpdater.getPose().getHeading()- Math.PI / 2);
                ...
                if(mShallEnd) {
                    ...
                    stopMotors();
                    FtcDashboard.getInstance().addConfigVariable(LateralZeroPowerAccelerationTuning.class.getSimpleName(),"SAVE",mShallSave);

                    if(mShallSave.get()) {
                        FollowerConstants.lateralZeroPowerAcceleration  = average;
                    }
                }
            ...
            mConfiguration.write(Environment.getExternalStorageDirectory().getPath()
                    + "/FIRST/lateral-zero-acceleration-tuning.json");
            mLogger.info("Updated configuration saved. You may retrieve it using <b>adb pull /sdcard/FIRST/lateral-zero-acceleration-tuning.json</b>");
            mLogger.update();
            mLogger.stop();
        }
        catch(Exception e) {
